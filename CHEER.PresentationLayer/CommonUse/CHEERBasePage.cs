...
using CHEER.Common.HtmlHelper.HtmlParser;
using System.Reflection;
using CHEER.CommonLayer.Common;
...
namespace CHEER.PresentationLayer
{
    ...
    /// CHEERBasePage 的摘要说明。
    /// </summary>
    public class CHEERBasePage : System.Web.UI.Page
    {
        #region 省市县
        ...
        protected static bool ValidateImageType(string fileName)
        {
            string fileType = String.Empty;
            int lastDotIndex = fileName.LastIndexOf(".");
            if (lastDotIndex >= 0)
        ...
        protected static bool ValidateZipType(string fileName)
        {
            string fileType = String.Empty;
            int lastDotIndex = fileName.LastIndexOf(".");
            if (lastDotIndex >= 0)
            ...
            if (VALID_ZIP_TYPES.Contains(fileType))
            {
                return true;
            }
            else
        ...
        protected static bool ValidateP12Type(string fileName)
        {
            string fileType = String.Empty;
            int lastDotIndex = fileName.LastIndexOf(".");
            if (lastDotIndex >= 0)
            ...
            if (VALID_P12_TYPES.Contains(fileType))
            {
                return true;
            }
            else
        ...
        protected readonly static List<string> VALID_FILE_TYPES3 = new List<string> { "doc", "docx", "xls", "xlsx", "rar", "ppt", "txt", "ai", "cdr", "avi", "mpg", "zip" };

        protected readonly static List<string> VALID_VIDEO_TYPES = new List<string> { "mp4", "mp3", "ogg" };

        protected static bool ValidateVideoFileType(string fileName)
        {
            string fileType = String.Empty;
            int lastDotIndex = fileName.LastIndexOf(".");
            if (lastDotIndex >= 0)
            ...
            if (VALID_VIDEO_TYPES.Contains(fileType))
            {
                return true;
            }
            else
        ...
        public void BindPagedDataTable(string sql, CheerUI.Grid grdMain, Platform.DAL.PersistBroker _broker = null)
        {
            Platform.DAL.PersistBroker broker = _broker;
            if (broker == null)
            {
                broker = Platform.DAL.PersistBroker.Instance();
            }
            try
            {
                if (sql.ToUpper().IndexOf("SQL_CALC_FOUND_ROWS") == -1)
                {
                    Regex regex = new Regex("SELECT", RegexOptions.IgnoreCase);
                    sql = regex.Replace(sql, "SELECT SQL_CALC_FOUND_ROWS ", 1);
                }
                sql += " ;SELECT FOUND_ROWS();";
                var ds = broker.ExecuteSQLForDst(sql);
                grdMain.DataSource = ds.Tables[0];
                grdMain.DataBind();
                grdMain.RecordCount = int.Parse(ds.Tables[1].Rows[0][0].ToString());
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private CHEER.Common.InfoPackage _InfomationPackage;
        /// <summary>
        /// 
        /// </summary>
        public CHEER.Common.InfoPackage InfomationPackage
        {
            get
            {
                if (System.Object.Equals(_InfomationPackage, null) == true)
                {
                    _InfomationPackage = GetBaseInfoPackage(GetSecurityChecker());
                }
                return _InfomationPackage;

            }
        }

        /// <summary>
        /// 替换'右尖括号','左尖括号'和联接符号等,保证值能被正确的在页面显示
        /// </summary>
        /// <param name="strValue">需要被替换的值</param>
        /// <returns>被替换后的值</returns>
        public static string RemedyStringForShow(string strValue)
        {
            if (strValue == null)
            {
                return "";
        ...
        public Hashtable GetEffectSecurityChecker()
        {
            if (Application[EFFECT_SECURITY_CHECKER_APPLICATION_STR] == null)
            {
                Application[EFFECT_SECURITY_CHECKER_APPLICATION_STR] = new Hashtable();
            }
            try
            {
                return (Hashtable)Application[EFFECT_SECURITY_CHECKER_APPLICATION_STR];
            }
            catch
            {
                Application[EFFECT_SECURITY_CHECKER_APPLICATION_STR] = new Hashtable();
        ...
        private static string EFFECT_SECURITY_CHECKER_APPLICATION_STR = "EffectSecurityCheckerList";

        public void AddEffectUser(string userid)
        {
            if (userid != null && GetEffectSecurityChecker().ContainsKey(userid) == false)
            {
                GetEffectSecurityChecker().Add(userid, userid);
            }
        }

        public void InvalidUserSecurityChecker(string userid)
        {
            if (userid != null && GetEffectSecurityChecker().ContainsKey(userid))
            {
                GetEffectSecurityChecker().Remove(userid);
            }
        }
        ...
        /// 重载方法OnInit
        /// </summary>
        /// <param name="e">EventArgs实例</param>
        override protected void OnInit(EventArgs e)
        {
            ...
            {
                Thread.CurrentThread.CurrentCulture = (CultureInfo)Session["CultureInfo"];
                Thread.CurrentThread.CurrentUICulture = (CultureInfo)Session["CultureInfo"];
            }
            base.OnInit(e);
        }
        ...
        public CHEER.Platform.GlobalRM.GlobalResourceManager _rm = null;
        public CHEER.Platform.GlobalRM.GlobalResourceManager getRM()
        {
            if (_rm == null)
            {
        ...
        public string getAlert(string _ID)
        {
            string str = getRM().GetLanString(_ID, "BU");
            return str;
        }
        ...
        public void Script(string _scriptContent)
        {
            if (_scriptContent.Trim() == "")
            {
                return;
            }
            Response.Write("<script language='javascript'>" + _scriptContent + "</script>");
        }

        public void ShowAlert(string _strMessage)
        {
            CheerUI.Alert.ShowInTop(_strMessage);
        }

        protected virtual void CheckIsLoginOut()
        {
            string _type = this.GetType().ToString();
            if (IsCheckLoginOut == true)
            {
                object _obj;
                _obj = this.Session[CHEER.Common.AppConstants.SystemAppConstants.SESSION_USERID];
                if (_obj == null)
                {
                    ...
                    _script = "top.C.notify({'message':'亲,已超时,请重新登录!','positionX':'center','positionY':'top'});top.showReloginWindow();";
                    if (CheerUI.PageManager.Instance != null && CheerUI.PageManager.Instance.IsCheerUIAjaxPostBack)
                    {
                        Response.Write(_script);
                    }
                    else
                    {
                        this.Script("top.location.replace('" + ResolveUrl("~/Default.aspx") + "');");
                    }
                ...
                {
                    //如果session没有过期，重新设置session以重新计算时间
                    this.Session[CHEER.Common.AppConstants.SystemAppConstants.SESSION_USERID] = _obj;
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            CheckIsLoginOut();
            base.OnLoad(e);
            if (CheerUI.PageManager.Instance != null)
            {
                var grid = (CheerUI.Grid)CheerUI.ControlUtil.FindControl(typeof(CheerUI.Grid));
                if (grid != null)
                {
                    grid.EnableColumnResize = true;
                }
                CheerUI.PageManager.Instance.FormMessageTarget = CheerUI.MessageTarget.Qtip;
                CheerUI.PageManager.Instance.AjaxLoadingType = CheerUI.AjaxLoadingType.Mask;
                CheerUI.PageManager.Instance.Theme = CheerUI.Theme.Bootstrap;
            }
        }

        protected virtual bool IsCheckLoginOut
        {
            get
            {
                return true;
        ...
        private SecurityChecker _securityChecker
        {
            get
            {
                if (this.Session["SecurityChecker"] == null)
                {
                    return null;
                }
                else
                {
                    return (SecurityChecker)Session["SecurityChecker"];
                }
            }
            set
            {
                this.Session["SecurityChecker"] = value;
            }
        }
        public SecurityChecker GetSecurityChecker()
        {
            string _userID = GetSession(CHEER.Common.AppConstants.SystemAppConstants.SESSION_USERID).ToString();
            if (GetEffectSecurityChecker().Contains(_userID) == false)
            {
                _securityChecker = null;
            }
            try
            {
                if (_securityChecker == null || _securityChecker.UserID != _userID)
                {
                    _securityChecker = new SecurityChecker(_userID);
                    AddEffectUser(_userID);
                }
        ...
        public SecurityChecker GetSecurityChecker(string userID)
        {
            SecurityChecker _checker = null;
            try
            {
                _checker = new SecurityChecker(userID);
                return _checker;
            }
            catch (Exception ex)
            {
        ...
        {
            object _obj;
            _obj = this.Session[sessionID];
            if (_obj == null)
            {
                Response.BufferOutput = true;
                Response.Clear();
                var _script = "top.C.notify({'message':'亲,已超时,请重新登录!','positionX':'center','positionY':'top'});top.showReloginWindow();";
                if (CheerUI.PageManager.Instance != null && CheerUI.PageManager.Instance.IsCheerUIAjaxPostBack)
                {
                    Response.Write(_script);
                }
        ...
        public PageAccessibility CheckAccessibilityByID(string functionID)
        {
            PageAccessibility _result = PageAccessibility.Unaccessible;
            try
            {
                //判断页面是否可访问
                if (GetSecurityChecker().IsAllow(functionID))
                    _result = PageAccessibility.Accessible;
                return _result;
            }
            catch
            {
                this.ShowAlert("在判断权限的时候发生错误，请检查权限设定是否正确！");
                return _result;
            }
        }

        public object CreatePalauObject(System.Type objType)
        {
            return eHRPageServer.GetPalauObject(objType);
        }

        public string getBusinessUnitID()
        {
            return Session[CHEER.Common.AppConstants.SystemAppConstants.SESSION_CURRENTMANAGEUNIT].ToString();
        }

        public Language GetLanguageInfo()
        {
            Language lang = Language.CN;
            return lang;
        }

        public object GetPalauObject(System.Type objType, InfoPackage infoPackage)
        {
            object palauObj = CreatePalauObject(objType);
            ((eHRLogicClass)palauObj).InformationPackage = infoPackage;
            return palauObj;
        }

        public object GetPalauObject(System.Type objType, string functionID, bool isAboutPerson, string pathField, bool isHaveManageUnit, string manageUnitID)
        {
            return GetPalauObject(objType, GetSecurityInfo(functionID, isAboutPerson, pathField, isHaveManageUnit, manageUnitID));
        }
        public object GetPalauObject(System.Type objType, string functionID, bool isAboutPerson, string pathField, bool isHaveManageUnit, string manageUnitID, bool isIncludeStopUnit)
        {
            return GetPalauObject(objType, GetSecurityInfo(functionID, isAboutPerson, pathField, isHaveManageUnit, manageUnitID, isIncludeStopUnit));
        }

        private InfoPackage GetBaseInfoPackage(SecurityChecker checker)
        {
            InfoPackage package = new InfoPackage();
            package.DBType = new SqlEntityHelp().GetDBType();
            package.UserID = checker.UserID;
            package.UserName = checker.User.UserName;
            package.PersonID = checker.User.PersonID;
            package.PersonName = checker.User.PersonName;
            package.BranchId = checker.User.OwnBranch.BranchID;
            package.BranchLabel = checker.User.OwnBranch.Lable;
            package.ManageUnitID = getBusinessUnitID();
            package.IsAboutPerson = true;
            package.LanguageCulture = "ZH-CN";
            return package;
        }

        public InfoPackage GetSecurityInfo(string functionID, bool isAboutPerson, string pathField, bool isHaveManageUnit, string manageUnitID, bool isIncludeStopUnit)
        {
            SecurityChecker checker = GetSecurityChecker();
            InfoPackage package = new InfoPackage();
            package.IsAboutPerson = isAboutPerson;
            package.UserID = checker.UserID;
            package.UserName = checker.User.UserName;
            package.PersonID = checker.User.PersonID;
            package.PersonName = checker.User.PersonName;
            package.ClientIP = Request.UserHostAddress;
            package.FunctionID = functionID;
            package.PathField = pathField;
            package.IsHaveManageUnit = isHaveManageUnit;
            package.ManageUnitID = manageUnitID;
            package.IsIncludeStopUnit = isIncludeStopUnit;
            if (isAboutPerson == true)
            {
                package.FilterSql = checker.GetFilterSql(functionID);
            }
            else
            {
                package.FilterSql = checker.GetBranchesFilterSql(functionID);
            }
            return package;
        }
        public InfoPackage GetSecurityInfo(string functionID, bool isAboutPerson, string pathField, bool isHaveManageUnit, string manageUnitID)
        {
            return GetSecurityInfo(functionID, isAboutPerson, pathField, isHaveManageUnit, manageUnitID, false);
        }

        public const string ErrorTransID = "ERROR_TransID";

        public void eHRThrow_Sys(Exception ErrorObject)
        {
            this.eHRThrow(ErrorObject, ExceptionLevel.System);
        }

        private void eHRThrow(Exception ErrorObject, ExceptionLevel ex_lv)
        {
            if (ex_lv == ExceptionLevel.System)
            {
                this.eHRThrow(ErrorObject, LogType_Error.Event);
            }
            else
            {
                throw ErrorObject;
        ...
        private string get_error_string(string err_origin)
        {
            if (err_origin.IndexOf("ERRORID:") == 0)
            {
                err_origin = err_origin.Substring(8, err_origin.Length - 8);
            }
            if (err_origin.Length >= 8)
            {
                string str_add = err_origin.Substring(0, err_origin.Length - 8);
                string str_check = err_origin.Substring(err_origin.Length - 8, 8);
                string str_show = "";
                switch (str_check)
                {
                    case "COMDESC1":
                        str_show = "描述:对数组进行索引出现异常.";//" 描述:对数组进行索引出现异常.";
                        break;
                    case "COMDESC2":
                        str_show = "描述:引用空对象.";//" 描述:引用空对象.";
                        break;
                    case "COMDESC3":
                        str_show = "描述:操作失败,无效状态信息.";//" 描述:操作失败,无效状态信息."; 
                        break;
                    case "COMDESC4":
                        str_show = "描述:参数出现异常.";//" 描述:参数出现异常.";
                        break;
                    case "COMDESC5":
                        str_show = "描述:在运行库的外部环境出现异常.";//" 描述:在运行库的外部环境出现异常.";
                        break;
                    case "COMDESC6":
                        str_show = "描述:系统异常.";//" 描述:系统异常.";
                        break;
                    default:
                        str_show = str_check;
                        break;
                }
                err_origin = str_add + str_show;
            }
            return "系统错误" + "\r\n\t" + "错误编号:" + err_origin + "\r\n\t" + "请联系系统管理员.";
        }

        public void eHRThrow(Exception ErrorObject, LogType_Error ErrorLevel)
        {
            //生成错误友好ID，初始化例外错误信息，记录日志。
            string str_show;
            string error_msg = "";
            Session[ErrorTransID] = null;
            if (ErrorObject.Message.IndexOf("ERRORID:") == 0)
            {
                error_msg = ErrorObject.Message;
            }
            else
            {
                bool flag;
                string errorID = ErrorFactory.GetErrorFriendlyIDByEx(ErrorObject, out str_show, out flag);
                error_msg = "ERRORID:" + errorID + str_show;
                Session[ErrorTransID] = errorID;
            }
            error_msg = get_error_string(error_msg);
        }
    }
}
